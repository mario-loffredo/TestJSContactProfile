...
public class LocalizationsTest {
    ...
    public void testLocalizeCard() throws IOException {
        ...
        assertEquals("testLocalizedCard - 1", jsCard.getName().getFull(), "VВася Пупкин");
        assertEquals("testLocalizedCard - 2", jsCard.getOrganizations().get("org").getName(), "Моя Компания");
        assertFalse("testLocalizedCard - 3", jsCard.getAddresses().isEmpty());
        assertEquals("testLocalizedCard - 4", jsCard.getAddresses().size(), 1);
        assertEquals("testLocalizedCard - 5", jsCard.getAddresses().get("addr").getComponents().length, 3);
        assertEquals("testLocalizedCard - 6", jsCard.getAddresses().get("addr").getComponents()[0].getKind(), "name");
        assertEquals("testLocalizedCard - 7", jsCard.getAddresses().get("addr").getComponents()[0].getValue(), "1, Улица");
        assertEquals("testLocalizedCard - 8", jsCard.getAddresses().get("addr").getComponents()[1].getKind(), "postOfficeBox");
        assertEquals("testLocalizedCard - 9", jsCard.getAddresses().get("addr").getComponents()[1].getValue(), "01001");
        assertEquals("testLocalizedCard - 10", jsCard.getAddresses().get("addr").getComponents()[2].getKind(), "locality");
        assertEquals("testLocalizedCard - 11", jsCard.getAddresses().get("addr").getComponents()[2].getValue(), "Киев");
        assertEquals("testLocalizedCard - 12", jsCard.getAddresses().get("addr").getCountryCode(), "UA");
